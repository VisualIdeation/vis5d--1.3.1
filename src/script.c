...
static Tcl_Interp *Tcl_CreateInterp( void )
{
   Tcl_Interp *interp;
/* 31Jan06  Phil McDonald */
   interp = (Tcl_Interp *) TMP_CALLOC (1, sizeof (Tcl_Interp));
/* end PM */
   if (interp) {
      interp->p = (struct private *) TMP_CALLOC (1, sizeof (struct private));
...
int	graphic_num (char str[])
{

    if (str == NULL) return VIS5D_BAD_CONSTANT;

    if (strcmp (str, "VIS5D_ISOSURF") == 0) return VIS5D_ISOSURF;
    if (strcmp (str, "VIS5D_HSLICE") == 0) return VIS5D_HSLICE;
    if (strcmp (str, "VIS5D_VSLICE") == 0) return VIS5D_VSLICE;
    if (strcmp (str, "VIS5D_CHSLICE") == 0) return VIS5D_CHSLICE;
    if (strcmp (str, "VIS5D_CVSLICE") == 0) return VIS5D_CVSLICE;
    if (strcmp (str, "VIS5D_VOLUME") == 0) return VIS5D_VOLUME;
    if (strcmp (str, "VIS5D_HWIND") == 0) return VIS5D_HWIND;
    if (strcmp (str, "VIS5D_VWIND") == 0) return VIS5D_VWIND;
    if (strcmp (str, "VIS5D_HSTREAM") == 0) return VIS5D_HSTREAM;
    if (strcmp (str, "VIS5D_VSTREAM") == 0) return VIS5D_VSTREAM;
    if (strcmp (str, "VIS5D_TRAJ") == 0) return VIS5D_TRAJ;
    if (strcmp (str, "VIS5D_TOPO") == 0) return VIS5D_TOPO;
    if (strcmp (str, "VIS5D_MAP") == 0) return VIS5D_MAP;
    if (strcmp (str, "VIS5D_BOX") == 0) return VIS5D_BOX;
    if (strcmp (str, "VIS5D_BACKGROUND") == 0) return VIS5D_BACKGROUND;
    if (strcmp (str, "VIS5D_LABEL") == 0) return VIS5D_LABEL;
    if (strcmp (str, "VIS5D_CLOCK") == 0) return VIS5D_CLOCK;
    if (strcmp (str, "VIS5D_PROBE") == 0) return VIS5D_PROBE;
...
static int cmd_enable_sfc_map (ClientData client_data, Tcl_Interp *interp,
                               int argc, char *argv[] )
{
   int	index, mode, result;




   if (!arg_check( interp, "vis5d_enable_sfc_map", argc, 2, 2 ))
   {
      return TCL_ERROR;
   }
   ...
   else if (strcmp(argv[2],"VIS5D_GET")==0) {
      mode = VIS5D_GET;
   }
   else {
      interp->result = "Error in vis5d_enable_sfc_map: bad mode";
      return TCL_ERROR;
   }

   result = vis5d_enable_sfc_map( index, mode );
   if (result >= 0)
   {
      interp->result = result ? "1" : "0";
      return TCL_OK;
   }
   return error_check( interp, "vis5d_enable_sfc_map", result );
}
/* end PM */
                               ...
                               int argc, char *argv[] )
{
   ...
   {
/* 31Jan06  Phil McDonald */
       vars = (int *) TMP_MALLOC (numvars * sizeof (int));
/* end PM */
...
static int cmd_make_box( ClientData client_data, Tcl_Interp *interp,
                         int argc, char *argv[] )
{
   int result;
   if (!arg_check( interp, "vis5d_make_box", argc, 4, 4 )) {
      return TCL_ERROR;
   }
   result = vis5d_make_box( atoi(argv[1]), atof(argv[2]),
                            atof(argv[3]), atof(argv[4]) );
   if (result == 0)
   {
      result = vis5d_load_topo_and_map (atoi (argv[1]));
   }
   return error_check( interp, "vis5d_make_box", result );
}
...
/* 17Aug07  Phil McDonald */
   int      idisp_ctx, result;
   int      uvar0, vvar0, wvar0;
   int      uvar1, vvar1, wvar1;
   int      uvart, vvart, wvart;
   ...
   idisp_ctx = atoi (argv[1]);

   uvar0 = varnum (idisp_ctx, argv[2]);
   vvar0 = varnum (idisp_ctx, argv[3]);
   wvar0 = varnum (idisp_ctx, argv[4]);
   uvar1 = varnum (idisp_ctx, argv[5]);
   vvar1 = varnum (idisp_ctx, argv[6]);
   wvar1 = varnum (idisp_ctx, argv[7]);
   uvart = varnum (idisp_ctx, argv[8]);
   vvart = varnum (idisp_ctx, argv[9]);
   wvart = varnum (idisp_ctx, argv[10]);

   result = vis5d_set_wind_vars (idisp_ctx,
                                 idisp_ctx, uvar0,
                                 idisp_ctx, vvar0,
                                 idisp_ctx, wvar0,
                                 idisp_ctx, uvar1,
...
/* 17Aug07  Phil McDonald */
   int      idisp_ctx, result;
   int      uvar0, vvar0, wvar0, uown0, vown0, wown0;
   int      uvar1, vvar1, wvar1, uown1, vown1, wown1;
   int      uvart, vvart, wvart, uownt, vownt, wownt;
   ...
   uown0 = atoi (argv[2]);
   uvar0 = varnum (uown0, argv[3]);
   vown0 = atoi (argv[4]);
   vvar0 = varnum (vown0, argv[5]);
   wown0 = atoi (argv[6]);
   wvar0 = varnum (wown0, argv[7]);
   uown1 = atoi (argv[8]);
   uvar1 = varnum (uown1, argv[9]);
   vown1 = atoi (argv[10]);
   vvar1 = varnum (vown1, argv[11]);
   wown1 = atoi (argv[12]);
   wvar1 = varnum (wown1, argv[13]);
   uownt = atoi (argv[14]);
   uvart = varnum (uownt, argv[15]);
   vownt = atoi (argv[16]);
   vvart = varnum (vownt, argv[17]);
   wownt = atoi (argv[18]);
   wvart = varnum (wownt, argv[19]);

   result = vis5d_set_wind_vars (idisp_ctx,
                                 uown0, uvar0, vown0, vvar0, wown0, wvar0,
                                 uown1, uvar1, vown1, vvar1, wown1, wvar1,
                                 uownt, uvart, vownt, vvart, wownt, wvart);
/* end PM */
...
/* 17Jan08  Phil McDonald */
   ...
   {
      var = varnum (idata_ctx, argv[2]);
   }
   result = vis5d_set_topo_color_var (atoi (argv[1]), idata_ctx, var);
/* end PM */
...
static int cmd_set_topo_hires_maxverts (ClientData client,
                                        Tcl_Interp *interp,
                                        int argc, char *argv[])
{
    ...
    if (!arg_check( interp, "vis5d_set_topo_hires_maxverts", argc, 2, 2 ))
    {
        return TCL_ERROR;
    }


    result = vis5d_set_topo_hires_maxverts (atoi (argv[1]), atoi (argv[2]));


    return error_check (interp, "vis5d_set_topo_hires_maxverts", result);
}
/* end PM */
...
static int cmd_set_volume_dims (ClientData client_data, Tcl_Interp *interp,
                                int argc, char *argv[])
{
   int	result;
   ...
   if (!arg_check( interp, "vis5d_set_volume_dims", argc, 4, 4 ))
   {
      return TCL_ERROR;
   }
   result = vis5d_set_volume_dims (atoi(argv[1]), atoi(argv[2]),
                                   atoi(argv[3]), atoi(argv[4]));
   return error_check (interp, "vis5d_set_volume_dims", result);
}
/* end PM */
...
/* 28Mar07  Phil McDonald */
   index   = atoi (argv[1]);
   graphic = graphic_num (argv[2]);
   if (graphic < 0) return TCL_ERROR;
/* end PM */
   if (argc >6){
                         ...
                         int argc, char *argv[] )
{
   ...
   sprintf( interp->result, "%g %g %g %g %g %g %g", xrot, yrot, zrot, scale,
                         ...
                         int argc, char *argv[] )
{
   ...
   sprintf( interp->result, "%g %g %g", scalex, scaley, scalez);
...
int	cmd_get_vert_exaggeration (ClientData client_data,
                                   Tcl_Interp *interp,
                                   int argc, char *argv[])
{
    int		result;
    ...
    if (!arg_check (interp, "vis5d_get_vert_exaggeration", argc, 1, 1))
    {
        return TCL_ERROR;
    }

    result = vis5d_get_vert_exaggeration (atoi (argv[1]), &sz);
    ...
    return error_check (interp, "vis5d_get_vert_exaggeration", result);
...
int	cmd_set_vert_exaggeration (ClientData client_data,
                                   Tcl_Interp *interp,
                                   int argc, char *argv[])
{
    int		result;

    if (!arg_check (interp, "vis5d_set_vert_exaggeration", argc, 2, 2))
    {
        return TCL_ERROR;
    }

    vis5d_set_vert_exaggeration (atoi (argv[1]), atof (argv[2]));
...
/* 31Jan06  Phil McDonald */
   nv        = TMP_CALLOC (1, sizeof (NameValue));
   nv->name  = TMP_STRDUP (argv[1]);
   nv->value = TMP_STRDUP (argv[2]);
/* end PM */
...
/* 05Aug07  Phil McDonald	make the register funcs external */
void register_vis5d_gui_commands( Tcl_Interp *interp )
/* end PM */
{
 ...
 */
/* 05Aug07  Phil McDonald	make the register funcs external */
void register_api_commands( Tcl_Interp *interp )
/* end PM */
{
#define REGISTER( NAME, FUNC ) Tcl_CreateCommand(interp,NAME,FUNC,NULL,NULL)
   ...
   REGISTER( "vis5d_make_box", cmd_make_box );
/* end PM */
...
/* 30Mar07  Phil McDonald */
   REGISTER( "vis5d_set_volume_dims", cmd_set_volume_dims );
/* end PM */
...
/* 10Sep07  Phil McDonald */
   REGISTER( "vis5d_enable_sfc_map", cmd_enable_sfc_map );
/* end PM */
...
/* 21Mar07  Phil McDonald */
   REGISTER("vis5d_get_vert_exaggeration", cmd_get_vert_exaggeration);
   REGISTER("vis5d_set_vert_exaggeration", cmd_set_vert_exaggeration);

/* 24Oct07  Phil McDonald */
   REGISTER("vis5d_set_topo_hires_maxverts", cmd_set_topo_hires_maxverts);
/* end PM */
...
/* 17Nov05  Phil McDonald */
      {
          char        *p_tmp;
          if ((p_tmp = strrchr (cmd, '\n')) != NULL) *p_tmp = '\0';
      }
/* end PM */
