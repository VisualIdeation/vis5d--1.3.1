...
/* 15Dec05  Phil McDonald	I think cb_chvvar is what we want here */
/*
...
*/
    vis5d_get_chslice (vindex, cb_chvvar[cb_dindex], &cur_lev);
    vis5d_set_chslice (vindex, cb_chvvar[cb_dindex], lev);
    ...
    for (time = 0; time < numtimes; time++)
    {
       vis5d_make_chslice (vindex, time, cb_chvvar[cb_dindex], time==curtime);
    }
    move_linked_hslices( vindex, dindex, VIS5D_CHSLICE, cb_chvvar[cb_dindex], lev);
...
static int interpret_cb( LUI_NEWBUTTON *b )
{
   ...
   char *dpy_name;
   GuiContext gtx = get_gui_gtx(b->context_index);
/* 14Dec05  Phil McDonald	TODO: this is a kludge */
   int		bindex = b->context_index;
/* end PM */
...
/* 14Dec05  Phil McDonald	something somewhere changes b->context_index */
   b->context_index = bindex;
/* end PM */
