...
/* 01Feb06  Phil McDonald */
	if (fdb_table!=NULL) TMP_FREE (fdb_table);
	fdb_table = (FileDB **) TMP_CALLOC (MAX_FDBS, sizeof (FileDB *));
/* end PM */
	/*
...
FileDB new_fdb( void )
{
   FileDB fdb;
/* 01Feb06  Phil McDonald */
   fdb = (FileDB) TMP_CALLOC (1, sizeof (struct file_db));
/* end PM */
   if (!fdb){
...
/* 01Feb06  Phil McDonald */
   yo = (struct file_info *) TMP_CALLOC (1, sizeof (struct file_info));
/* end PM */
   if (!yo){
...
/* 01Feb06  Phil McDonald */
      recids[i] =  (int *) TMP_MALLOC (sizeof (int) * num_recs[i]);
/* end PM */
      if (!recids){
...
/* end PM */
   ...
   for (i = 0; i < num_times; i++){
/* 01Feb06  Phil McDonald */
      F->RecID[i] = (int *) TMP_MALLOC (sizeof (int) * num_recs[i]);
/* end PM */
      if (!F->RecID){
...
int load_fdb_into_v5d( int index, irregular_v5dstruct *iv )
{
   ...
   for (i = 0; i < fdb->NumTimes; i++){
      if (fdb->TimeSelected[i]){
/* 01Feb06  Phil McDonald */
         fdb->RecIDfileindex[j] = (int *) TMP_MALLOC
                                  (sizeof (int) * fdb->NumRecs[i]);
         fdb->RecIDfilerecid[j] = (int *) TMP_MALLOC
                                  (sizeof (int) * fdb->NumRecs[i]);
/* end PM */
