...
#define DISTANCE(r1,g1,b1,r2,g2,b2)  ( ((r2)-(r1)) * ((r2)-(r1)) + \
   ...
   if (!XAllocColor(dpy, cmap, color)) {
      ...
      if (!allcolors) {
/* 31Jan06  Phil McDonald */
         allcolors = (XColor *) TMP_MALLOC (cmap_size * sizeof (XColor));
/* end PM */
...
/* 31Jan06  Phil McDonald */
      dtx->Sound.vertdata = (float *) mem_malloc
                            (dtx->MaxNl * sizeof (float) + 1);
/* end PM */
...
/* 31Jan06  Phil McDonald */
   if (dtx->Sound.soundline != NULL){
      mem_free (dtx->Sound.soundline);
   }
   dtx->Sound.soundline = (float *) mem_malloc (nl * sizeof (float) );
/* end PM */
...
/* 31Jan06  Phil McDonald */
   if (ctx->dpy_ctx->Sound.soundline != NULL){
      mem_free (ctx->dpy_ctx->Sound.soundline);
   }
   ctx->dpy_ctx->Sound.soundline = (float *) mem_malloc (nl * sizeof (float));
/* end PM */
...
/* 31Jan06  Phil McDonald */
   if (dtx->Sound.uwindline != NULL) mem_free (dtx->Sound.uwindline);
   if (dtx->Sound.vwindline != NULL) mem_free (dtx->Sound.vwindline);

   dtx->Sound.uwindline = (float *) mem_malloc (nl * sizeof (float));
   dtx->Sound.vwindline = (float *) mem_malloc (nl * sizeof (float));
   if ((!dtx->Sound.uwindline) || (!dtx->Sound.vwindline)) {

      /* MJK 12.15.98 */
      if (dtx->Sound.uwindline != NULL)
      {
         mem_free (dtx->Sound.uwindline);
         dtx->Sound.uwindline = NULL;
      }
      if (dtx->Sound.vwindline != NULL)
      {
         mem_free (dtx->Sound.vwindline);
         dtx->Sound.vwindline = NULL;
      }
...
/* 31Jan06  Phil McDonald */
   if (ctx->dpy_ctx->Sound.uwindline != NULL)
      mem_free (ctx->dpy_ctx->Sound.uwindline);
   if (ctx->dpy_ctx->Sound.vwindline != NULL)
      mem_free (ctx->dpy_ctx->Sound.vwindline);

   ctx->dpy_ctx->Sound.uwindline = (float *) mem_malloc (nl * sizeof (float));
   ctx->dpy_ctx->Sound.vwindline = (float *) mem_malloc (nl * sizeof (float));
