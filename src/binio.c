 ...
 * Updates:
 *
 * May 30, 2007    Phil McDonald
 *   changed int to size_t where appropriate to accommodate 64-bit
 *   architectures.
 *
 ...
 */
void flip4( const unsigned int *src, unsigned int *dest, size_t n )
{
   size_t i;
 ...
 */
ssize_t read_bytes( int f, void *b, size_t n )
{
   return read( f, b, n );
}
 ...
 */
ssize_t read_int2_array( int f, short *iarray, size_t n )
{
#ifdef _CRAY
   int i;
   signed char *buffer;
   int nread;
/* 01Feb06  Phil McDonald */
   buffer = (signed char *) TMP_MALLOC (n * 2);
/* end PM */
   if (!buffer)  return 0;
...
/* end PM */
   return nread;
#else
   ssize_t nread = read( f, iarray, n*2 );
   if (nread<=0)
      return 0;
 ...
 * Return:  number of integers read.
 */
ssize_t read_uint2_array( int f, unsigned short *iarray, size_t n )
{
#ifdef _CRAY
   int i;
   ...
   int nread;
/* 01Feb06  Phil McDonald */
   buffer = (unsigned char *) TMP_MALLOC (n * 2);
/* end PM */
   if (!buffer)  return 0;
   nread = read( f, buffer, n*2 );
   if (nread<=0)  return 0;
 ...
 * Return:  number of integers read.
 */
ssize_t read_int4_array( int f, int *iarray, size_t n )
{
#ifdef _CRAY
   int j, nread;
   int *buffer;
...
/* end PM */
   return nread;
#else
   ssize_t nread = read( f, iarray, 4*n );
   if (nread<=0)
     return 0;
#  ifndef WORDS_BIGENDIAN
 ...
 * Return:  1 = ok, 0 = error
 */
ssize_t read_float4( int f, float *x )
{
#ifdef _CRAY
   long buffer = 0;
 ...
 */
ssize_t read_float4_array( int f, float *x, size_t n )
{
#ifdef _CRAY
 ...
 */
ssize_t read_block( int f, void *data, size_t elements, int elsize )
{
   if (elsize==1) {
 ...
 */
ssize_t write_bytes( int f, const void *b, size_t n )
{
   return write( f, b, n );
}
 ...
 */
ssize_t write_int2_array( int f, const short *iarray, size_t n )
{
#ifdef _CRAY
 ...
 * Return:  number of integers written
 */
ssize_t write_uint2_array( int f, const unsigned short *iarray, size_t n )
{
#ifdef _CRAY
   int i, nwritten;
   unsigned char *buffer;
/* 01Feb06  Phil McDonald */
   buffer = (unsigned char *) TMP_MALLOC (2 * n);
/* end PM */
   if (!buffer)  return 0;
   for (i=0;i<n;i++) {
 ...
 * Return:  1 = ok, 0 = error
 */
ssize_t write_int4( int f, int i )
{
#ifdef _CRAY
   i = i << 32;
 ...
 *  Return:  number of integers written.
 */
ssize_t write_int4_array( int f, const int *i, size_t n )
{
#ifdef _CRAY
   int j, nwritten;
   char *buf, *b, *ptr;
 ...
 * Return:  1 = ok, 0 = error
 */
ssize_t write_float4( int f, float x )
{
#ifdef _CRAY
   char buffer[8];
...
#else
#  ifndef WORDS_BIGENDIAN
/* 08Mar06  Phil McDonald	TODO: buggy with amd64 gcc v4.0.2 */
/*
      float y;
...
*/
      return write_float4_array (f, &x, 1);
/* end PM */
#  else
      float y;
      y = (float) x;
 ...
 */
ssize_t write_float4_array( int f, const float *x, size_t n )
{
#ifdef _CRAY
...
#  ifndef WORDS_BIGENDIAN
      ssize_t nwritten;
/* 25Sep06  Phil McDonald	TODO: buggy with amd64 gcc v4.0.2 */
      fflush (stderr);
/* end PM */
 ...
 * Return: number of elements written
 */
ssize_t write_block( int f, const void *data, size_t elements, int elsize )
{
   if (elsize==1) {
