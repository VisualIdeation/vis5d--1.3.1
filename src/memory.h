...
/* 10Sep07  Phil McDonald	use size_t type for number of bytes */
extern int init_memory( Context ctx, size_t bytes );

extern int init_shared_memory( Context ctx, void *start, size_t bytes );

extern int reinit_memory( Context ctx );

#ifndef allocate
extern void *allocate( Context ctx, size_t bytes );
#endif

extern void *allocate_type( Context ctx, size_t bytes, int type );

extern void *pallocate( Context ctx, size_t bytes );

extern void deallocate( Context ctx, void *addr, size_t bytes );

extern size_t mem_available( Context ctx );

extern size_t mem_used( Display_Context dtx );
...
extern int      mem_set_funcs (void* (*calloc_func) (size_t nmemb, size_t size),
...
extern char*	mem_strdup (char* str);

/* 27Jan06  Phil McDonald       to help clarify temporary memory allocations */
#define TMP_MALLOC                      malloc
#define TMP_CALLOC                      calloc
#define TMP_REALLOC                     realloc
#define TMP_FREE                        free

/* 31Jan06  Phil McDonald */
#define TMP_STRDUP			strdup
/* end PM */
