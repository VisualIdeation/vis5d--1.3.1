...
static void skip( FILE *f, size_t n )
{
   if (n>0) {
      fseek( f, n, SEEK_CUR );
   }
 ...
 */
static void *alloc_and_read( Context ctx, FILE *f, size_t bytes )
{
   void *p;
...
static void restore_traj( Context ctx, FILE *f, size_t blocklength )
{
   int length, kind, group;
   size_t bytes;
   struct traj *t;
...
static void restore_hwind( Context ctx, FILE *f, size_t blocklength )
{
   int ws, it;
   size_t bytes;
...
#ifdef LEAVEOUT
static void restore_vwind( Context ctx, FILE *f, size_t blocklength )
{
   int ws, it;
   size_t bytes;
...
static void restore_stream( Context ctx, FILE *f, size_t blocklength )
{
   int ws, it;
   size_t bytes;
...
/* 01Feb06  Phil McDonald */
		ctx->Variable[var]->ExpressionList = (char *) mem_malloc (500);
/* end PM */
