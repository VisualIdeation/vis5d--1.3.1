...
#define GLBEGINNOTE  if(vis5d_verbose & VERBOSE_OPENGL) fprintf(stderr,"calling glbegin at line %d\n",__LINE__);
...
/* 08Jan07  Phil McDonald	off-screen rendering needs indirect rendering */
   dtx->gl_ctx = glXCreateContext (GfxDpy, visualinfo, NULL,
...
/* 16Jan08  Phil McDonald */
         if (current_dtx->GfxWindow == NULL) return;
/* end PM */
...
void set_3d( int perspective, float frontclip, float zoom, float *modelmat)
{
   ...
   eqntop[3] = -1 * current_dtx->VClipTable[0].eqn[3] + 0.01;
        check_gl_error("set_3d: top done");
   ...
   eqnbottom[3] =current_dtx->VClipTable[1].eqn[3] + 0.01;
        check_gl_error("set_3d: bottom done");
   ...
   eqnleft[3] = current_dtx->VClipTable[2].eqn[3] + 0.01;
        check_gl_error("set_3d: left done");
   ...
   eqnright[3] = -1 * current_dtx->VClipTable[3].eqn[3] + 0.01;
        check_gl_error("set_3d: right done");
   ...
   eqnfront[3] = current_dtx->HClipTable[0].eqn[3] + 0.01;
        check_gl_error("set_3d: front done");
   ...
   eqnback[3] = -1 * current_dtx->HClipTable[1].eqn[3] + 0.01;
        check_gl_error("set_3d: back done");
...
void set_color( unsigned int c )
{
   GLfloat material_color[4];
	...
	check_gl_error("set_color: begin");
   ...
   material_color[3] = UNPACK_ALPHA( c ) / 255.0;
	check_gl_error("set_color: material_color");
								 ...
								 GLuint *list, int listtype )
{
   ...
   glShadeModel( GL_FLAT );
   glDisable( GL_DITHER );
	if(vis5d_verbose & VERBOSE_OPENGL) fprintf(stderr,"draw_disjoint_lines %d\n",n);
