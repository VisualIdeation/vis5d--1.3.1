 ...
 */
static void *i_alloc( Irregular_Context itx, size_t b, int permanent, int type )
{
   size_t bytes;
   struct mem *pos, *new;
 ...
 */
static void i_dealloc( Irregular_Context itx, void *addr, size_t b )
{
   size_t bytes;
 ...
 */
int init_irregular_memory( Irregular_Context itx, size_t bytes )
{
   struct mem *m;
 ...
 * Return:  1 = success, 0 = error
 */
int i_init_shared_memory( Irregular_Context itx, void *start, size_t bytes )
{
   struct mem *m;
 ...
 * Input:  itx - the vis5d context
 */
size_t i_mem_available( Irregular_Context itx )
{
 ...
 */
void *i_allocate( Irregular_Context itx, size_t bytes )
{
   assert( bytes>=0 );
 ...
 * Return:  address of memory block or NULL if out of memory.
 */
void *i_allocate_type( Irregular_Context itx, size_t bytes, int type )
{
   assert( bytes>=0 );
 ...
 */
void *i_pallocate( Irregular_Context itx, size_t bytes )
{
   if (itx->memory_limit==0) {
      /* just malloc */
/* 27Jan06  Phil McDonald */
 ...
 */
void i_deallocate( Irregular_Context itx, void *addr, size_t bytes )
{
   LOCK_ON( itx->memlock );
   if (addr) {
      if (itx->memory_limit==0) {
