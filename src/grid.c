...
int open_gridfile( Context ctx, char filename[] )
{
   ...
   /* MJK 12.02.98 begin */
/* 03Apr06  Phil McDonald	to allow simultaneous use of Vis5D and    */
/*				user files, try to open a file as a Vis5D */
/*				first.  If that fails, try to open it as  */
/*				user file.                                */
   if (!initially_open_gridfile( filename, &ctx->G))
   {
      if (!ctx->UserDataFlag) return 0;
      if (!open_userfile (filename, &ctx->G)) return 0;
   }
/* end PM */
...
/* 27Jan06  Phil McDonald */
	  ctx->Variable[i] = (vis5d_variable *)
                             mem_calloc (1, sizeof (vis5d_variable));  
/* end PM */
   }
...
void free_grid_cache( Context ctx )
{
   ...
   for (it=0; it<MAXTIMES; it++){
	...
	for(it=0;it<ctx->MaxCachedGrids;it++)
	  deallocate(ctx,ctx->GridCache[it].Data, -1);
 ...
 */
int init_grid_cache( Context ctx, size_t maxbytes, float *ratio )
{
   int i, it, iv;
   int maxnl;
   size_t gridsize;
...
/* 03Apr06  Phil McDonald	to allow simultaneous use of Vis5D and  */
/*				user files, try to get a data grid from */
/*				a user file first.  If that fails, try  */
/*				to get it from a Vis5D file.            */
   ok = 0;
   if (ctx->UserDataFlag)
   {
...
/* 27Jan06  Phil McDonald */
	ctx->Variable[newvar] = (vis5d_variable *)
                                mem_calloc (1, sizeof (vis5d_variable));
/* end PM */
