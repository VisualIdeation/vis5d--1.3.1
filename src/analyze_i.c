...
/* 01Feb06  Phil McDonald */
/* end PM */
            db->VarNames[db->NumVars] = TMP_STRDUP (g->VarName);
            if (g->Units) {
               db->Units[db->NumVars] = TMP_STRDUP (g->Units);
            }
            db->NumVars++;
         }
         else {
...
/* 01Feb06  Phil McDonald */
   grid_array = (struct grid_info **) TMP_MALLOC
                (db->NumGrids * sizeof (struct grid_info *));
/* end PM */
