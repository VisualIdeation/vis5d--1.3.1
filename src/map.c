...
/* 27Jan06  Phil McDonald */
    xy = (FLOAT2 *) TMP_MALLOC (n_in * sizeof (FLOAT2));
/* end PM */
    if (xy == NULL) return 0;
                               ...
                               float *xyz_out)
{
    ...
    nnew = bend_line_to_fit_surf (dtx->topo->TopoVertex, dtx->topo->qcols, dtx->topo->qrows,
                                  ...
                                  dtx->topo->z_fuzz, xyz_in, n_in, xyz_out);
...
/* 27Jan06  Phil McDonald */
   dir = (int *) TMP_MALLOC (nsegs * 6 * sizeof(int));
/* end PM */
   if (!dir) {
      close( mapfile );
      return 0;
...
/* 27Jan06  Phil McDonald */
         verts = (int *) TMP_MALLOC (length * sizeof(int));
