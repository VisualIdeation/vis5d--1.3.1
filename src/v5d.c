 ...
 */
static ssize_t grid_position( const v5dstruct *v, int time, int var )
{
   int i;
   ssize_t pos;
 ...
 */
size_t v5dSizeofGrid( const v5dstruct *v, int time, int var )
{
...
/* 31Jan06  Phil McDonald */
   v = (v5dstruct *) TMP_MALLOC (sizeof (v5dstruct));
/* end PM */
   if (v) {
      v5dInitStruct(v);
   }
...
/* 31Jan06  Phil McDonald */
   compdata = (void *) TMP_MALLOC (bytes);
/* end PM */
   if (!compdata) {
...
static int write_tag( v5dstruct *v, int tag, size_t length, int newfile )
{
   if (!newfile) {
