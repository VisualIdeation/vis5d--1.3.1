...
/* 15Mar06  Phil McDonald */
#define RCL_FUZZ			0.005

static int	rcl_check (float *p_row, float *p_col, float *p_lev,
		           float rcl_max[3])
{

    if (rcl_max == NULL) return 0;


    if (*p_row < 0.0)
    {
        if (-(*p_row) > RCL_FUZZ) return 0;
        *p_row = 0.0;
    }
    else if (*p_row > rcl_max[0])
    {
        if ((*p_row - rcl_max[0]) > RCL_FUZZ) return 0;
        *p_row = rcl_max[0];
    }

    if (*p_col < 0.0)
    {
        if (-(*p_col) > RCL_FUZZ) return 0;
        *p_col = 0.0;
    }
    else if (*p_col > rcl_max[1])
    {
        if ((*p_col - rcl_max[1]) > RCL_FUZZ) return 0;
        *p_col = rcl_max[1];
    }
    ...
    else
    {
        if (*p_lev < 0.0)
        {
            if (-(*p_lev) > RCL_FUZZ) return 0;
            *p_lev = 0.0;
        }
        else if (*p_lev > rcl_max[2])
        {
            if ((*p_lev - rcl_max[2]) > RCL_FUZZ) return 0;
            *p_lev = rcl_max[2];
        }
    }
...
/* 15Mar06  Phil McDonald	allow for some r,c,l tolerance */
                 if (!rcl_check (&row, &col, &lev, rcl_max))
                 {
                     color_indexes[i] = 255;
                 }
                 else
...
/* 31Jan06  Phil McDonald */
   uint_1	*deci_color_indexes		= NULL;
/* end PM */
   size_t  n;
   ...
   if (colorvar!=-1) {
      ...
      size_t nbytes = ctx->Variable[isovar]->SurfTable[time]->numverts;
      color_indexes = allocate( ctx, nbytes*sizeof(uint_1) );
      if (!color_indexes) {
         return;
      }
		...
		compute_iso_colors(ctx, cvctx, colorvar, cvctx->Variable[colorvar]->MinVal, 
								 ...
								 color_indexes, nbytes);

      if (ctx->Variable[isovar]->SurfTable[time]->deci_verts) {
		  /* Allocate storage for new color indexes */
		  nbytes = ctx->Variable[isovar]->SurfTable[time]->deci_numverts;
		  deci_color_indexes = allocate( ctx, nbytes*sizeof(uint_1) );
...
/* 30Jan06  Phil McDonald */
      vc   = (float *) TMP_MALLOC (sizeof (float) * MAX_ISO_VERTS);
      vr   = (float *) TMP_MALLOC (sizeof (float) * MAX_ISO_VERTS);
      vl   = (float *) TMP_MALLOC (sizeof (float) * MAX_ISO_VERTS);
      vc2  = (float *) TMP_MALLOC (sizeof (float) * MAX_ISO_VERTS);
      vr2  = (float *) TMP_MALLOC (sizeof (float) * MAX_ISO_VERTS);
      vl2  = (float *) TMP_MALLOC (sizeof (float) * MAX_ISO_VERTS);
      nx   = (float *) TMP_MALLOC (sizeof (float) * MAX_ISO_VERTS);
      ny   = (float *) TMP_MALLOC (sizeof (float) * MAX_ISO_VERTS);
      nz   = (float *) TMP_MALLOC (sizeof (float) * MAX_ISO_VERTS);
      vpts = (int *) TMP_MALLOC (sizeof (int) * 2 * MAX_ISO_VERTS);
/* end PM */
...
/* end PM */
	...
	*/
	if (dtx->MaxTMesh > 0 && (numindexes - 2) > dtx->MaxTMesh) {
	  ...
	  size_t deci_vbytes, deci_nbytes;

	  size_t nbytes = (dtx->MaxTMesh + fudge) * 3 * sizeof(float);

	  printf("Entering Decimate\n");


	  deci_vr = allocate(ctx, nbytes);
	  deci_vc = allocate(ctx, nbytes);
	  deci_vl = allocate(ctx, nbytes);
	  deci_nx = allocate(ctx, nbytes);
	  deci_ny = allocate(ctx, nbytes);
	  deci_nz = allocate(ctx, nbytes);
	  ...
	  */
	  ...
	  deallocate(ctx, deci_vc, nbytes);
	  deallocate(ctx, deci_vr, nbytes);
	  deallocate(ctx, deci_vl, nbytes);
	  deallocate(ctx, deci_nx, nbytes);
	  deallocate(ctx, deci_ny, nbytes);
	  deallocate(ctx, deci_nz, nbytes);
	}
	else {
		deci_cverts = NULL;
		deci_cnorms = NULL;
...
/* 05Jun07  Phil McDonald */
static float	interp_sfc_val (Context ctx, int time, int var,
		                float lat, float lon, float rcl_max[])
{

    float	hgt, x, y, z, row, col, lev;



    hgt = elevation (ctx->dpy_ctx, ctx->dpy_ctx->topo, lat, lon, NULL) / 1000.0;

    geo_to_xyz (ctx, time, var, 1, &lat, &lon, &hgt, &x, &y, &z);
    xyz_to_grid (ctx, time, var, x, y, z, &row, &col, &lev);

    if (!rcl_check (&row, &col, &lev, rcl_max)) return MISSING;


    return interpolate_grid_value (ctx, time, var, row, col, lev);
}
/* end PM */
                                    ...
                                    float *grid, int colmajor)
{
    ...
    float       *slice_data, x, y, z, grow, gcol, glev, gbot, gtop, rcl_max[3];
    double      rr, cc, dr, dc;
    Display_Context     dtx = ctx->dpy_ctx;
...
/* 05Jun07  Phil McDonald	There were several problems with surface    */
/*				product objects when multiple data contexts */
/*				were involved.  The objects didn't seem to  */
/*				line up very well with their counterparts   */
/*				and some indexing issues caused seg faults. */
/*                                                                          */
/*				This method hasn't been tested on a lot of  */
/*				different cases, so it may need some more   */
/*				refinement.                                 */
#ifdef LEAVEOUT
...
/* end PM */
    ...
    if (colmajor)
    {
        for (ir = 0; ir < nrows; ir++)
        {
            ...
            for (ic = 0; ic < ncols; ic++)
            {
                rowcolPRIME_to_latlon (dtx, -1, -1, (float) ir, (float) ic,
                                       &thelat, &thelon);
                slice_data[di] = interp_sfc_val (ctx, time, var,
                                                 thelat, thelon, rcl_max);
                di += nrows;
            }
        }
    }
    else
    {
        ...
        for (ir = 0; ir < nrows; ir++)
        {
            for (ic = 0; ic < ncols; ic++)
            {
                rowcol_to_latlon (ctx, time, var, (float) ir, (float) ic,
                                  &thelat, &thelon);
                slice_data[di] = interp_sfc_val (ctx, time, var,
                                                 thelat, thelon, rcl_max);
                di++;
...
/* 30Jan06  Phil McDonald */
   ...
   /***************************/
   lat = (float *) TMP_MALLOC (sizeof (float) * itx->NumRecs[time]);
   lon = (float *) TMP_MALLOC (sizeof (float) * itx->NumRecs[time]);
   alt = (float *) TMP_MALLOC (sizeof (float) * itx->NumRecs[time]);

   xs  = (float *) TMP_MALLOC (sizeof (float) * itx->NumRecs[time]);
   ys  = (float *) TMP_MALLOC (sizeof (float) * itx->NumRecs[time]);
   zs  = (float *) TMP_MALLOC (sizeof (float) * itx->NumRecs[time]);

   vx  = (float *) TMP_MALLOC (sizeof (float) * MAX_TEXT_PLOT_VERTS);
   vy  = (float *) TMP_MALLOC (sizeof (float) * MAX_TEXT_PLOT_VERTS);
   vz  = (float *) TMP_MALLOC (sizeof (float) * MAX_TEXT_PLOT_VERTS);
   ...
   {
      tempcols = (uint_1 *) TMP_MALLOC (sizeof (uint_1) * MAX_TEXT_PLOT_VERTS);
...
/* 30Jan06  Phil McDonald */
   if (itx->Variable[itx->TextPlotVar]->VarType == NUMERICAL_VAR_1D){
      numdata = (double *) TMP_MALLOC (sizeof (double) * numtouse);
   }
   else if (itx->Variable[itx->TextPlotVar]->VarType == CHARACTER_VAR){
      chardata = (char *) TMP_MALLOC (sizeof (char) * numtouse *
                          itx->Variable[itx->TextPlotVar]->CharVarLength);
   }
   if (!numdata || !chardata)
   {
      printf("not enough memory in calc_textpot\n");
      if (!numdata) TMP_FREE (numdata);
      if (!chardata) TMP_FREE (chardata);
      if (!tempcols) TMP_FREE (tempcols);
      TMP_FREE (lat);
      TMP_FREE (lon);
      TMP_FREE (alt);
      TMP_FREE (xs);
...
/* 30Jan06  Phil McDonald */
   vr1 = (float *) TMP_MALLOC (sizeof (float) * max_cont_verts);
   vc1 = (float *) TMP_MALLOC (sizeof (float) * max_cont_verts);
   vr2 = (float *) TMP_MALLOC (sizeof (float) * max_cont_verts / 2);
   vc2 = (float *) TMP_MALLOC (sizeof (float) * max_cont_verts / 2);
   vr3 = (float *) TMP_MALLOC (sizeof (float) * max_cont_verts / 2);
   vc3 = (float *) TMP_MALLOC (sizeof (float) * max_cont_verts / 2);
   vl  = (float *) TMP_MALLOC (sizeof (float) * max_cont_verts);
...
/* 30Jan06  Phil McDonald */
         ...
         labels = (char *) mem_malloc (10 * sizeof (char) * max_cont_verts / 2);
#endif
...
/* 30Jan06  Phil McDonald */
   vr1 = (float *) TMP_MALLOC (sizeof (float) * max_cont_verts);
   vc1 = (float *) TMP_MALLOC (sizeof (float) * max_cont_verts);
   vl1 = (float *) TMP_MALLOC (sizeof (float) * max_cont_verts);
   vr2 = (float *) TMP_MALLOC (sizeof (float) * max_cont_verts / 2);
   vc2 = (float *) TMP_MALLOC (sizeof (float) * max_cont_verts / 2);
   vl2 = (float *) TMP_MALLOC (sizeof (float) * max_cont_verts / 2);
   vr3 = (float *) TMP_MALLOC (sizeof (float) * max_cont_verts / 2);
   vc3 = (float *) TMP_MALLOC (sizeof (float) * max_cont_verts / 2);
   vl3 = (float *) TMP_MALLOC (sizeof (float) * max_cont_verts / 2);
...
/* 30Jan06  Phil McDonald */
   vr = (float *) TMP_MALLOC (MAXROWS * MAXCOLUMNS * sizeof (float));
   vc = (float *) TMP_MALLOC (MAXROWS * MAXCOLUMNS * sizeof (float));
   vl = (float *) TMP_MALLOC (MAXROWS * MAXCOLUMNS * sizeof (float));
   if (!vr || !vc || !vl)
   {
...
/* 30Jan06  Phil McDonald */
   ...
   vr = (float *) TMP_MALLOC (MAXROWS * MAXLEVELS * sizeof (float));
   vc = (float *) TMP_MALLOC (MAXROWS * MAXLEVELS * sizeof (float));
   vl = (float *) TMP_MALLOC (MAXROWS * MAXLEVELS * sizeof (float));
#else
   vr = (float *) TMP_MALLOC (MAXCOLUMNS * MAXLEVELS * sizeof (float));
   vc = (float *) TMP_MALLOC (MAXCOLUMNS * MAXLEVELS * sizeof (float));
   vl = (float *) TMP_MALLOC (MAXCOLUMNS * MAXLEVELS * sizeof (float));
#endif
   if (!vr || !vc || !vl)
   {
...
/* 30Jan06 Phil McDonald */
   vr = (float *) TMP_MALLOC (sizeof (float) * MAX_WIND_VERTS);
   vc = (float *) TMP_MALLOC (sizeof (float) * MAX_WIND_VERTS);
   vl = (float *) TMP_MALLOC (sizeof (float) * MAX_WIND_VERTS);
   if (!vr || !vc || !vl)
   {
...
/* MJK 3.25.99 */
   ...
   if (vcount>0) {
      size_t bytes = 3*vcount*sizeof(int_2);
                        ...
                        float c1, float r2, float c2)
{
   ...
   if (dtx->CurvedBox==0){
/* 30Jan06  Phil McDonald */
      v = (float *) mem_malloc (5 * 3 * sizeof (float));
/* end PM */
      if (!v){
...
/* 30Jan06  Phil McDonald */
      v = (float *) mem_malloc
          (((2 * dtx->Nl) + (2 * dtx->Nc) - 3) * 3 * sizeof (float));
      if (!v){
         printf("error in calc_vclip\n");
         exit(1);
      }
...
/* 30Jan06  Phil McDonald */
      mem_free (dtx->VClipTable[num].boxverts);
/* end PM */
...
/* 30Jan06  Phil McDonald */
      v = (float *) mem_malloc
          (((2 * dtx->Nr) + (2 * dtx->Nc) - 3) * 3 * sizeof (float));
      if (!v){
         printf("error in calc_vclip\n");
         exit(1);
      }       
...
/* 30Jan06  Phil McDonald */
      mem_free (dtx->HClipTable[num].boxverts);
/* end PM */
...
/* end PM */
   ...
   if (num > 0) {
      size_t bytes = 3 * num * sizeof(int_2);
...
/* 30Jan06  Phil McDonald */
   ...
   vc  = (float *) TMP_MALLOC (sizeof (float) * MAX_WIND_VERTS);
   vl  = (float *) TMP_MALLOC (sizeof (float) * MAX_WIND_VERTS);
   vr2 = (float *) TMP_MALLOC (sizeof (float) * MAX_WIND_VERTS);
   vc2 = (float *) TMP_MALLOC (sizeof (float) * MAX_WIND_VERTS);
   vl2 = (float *) TMP_MALLOC (sizeof (float) * MAX_WIND_VERTS);
...
/* 30Jan06  Phil McDonald */
   vr = (float *) TMP_MALLOC (sizeof (float) * MAX_TRAJ_VERTS);
   vc = (float *) TMP_MALLOC (sizeof (float) * MAX_TRAJ_VERTS);
   vl = (float *) TMP_MALLOC (sizeof (float) * MAX_TRAJ_VERTS);

/* WLH 20 Oct 98
   vx = (float *) TMP_MALLOC (sizeof (float) * MAX_TRAJ_VERTS);
   vy = (float *) TMP_MALLOC (sizeof (float) * MAX_TRAJ_VERTS);
   vz = (float *) TMP_MALLOC (sizeof (float) * MAX_TRAJ_VERTS);
   nx = (float *) TMP_MALLOC (sizeof (float) * MAX_TRAJ_VERTS);
   ny = (float *) TMP_MALLOC (sizeof (float) * MAX_TRAJ_VERTS);
   nz = (float *) TMP_MALLOC (sizeof (float) * MAX_TRAJ_VERTS);
   tt = (int *) TMP_MALLOC (sizeof (int) * MAX_TRAJ_VERTS);
*/
   /* WLH 20 Oct 98 */
   if (ribbon) {
     vx = (float *) TMP_MALLOC (2 * sizeof (float) * MAX_TRAJ_VERTS);
     vy = (float *) TMP_MALLOC (2 * sizeof (float) * MAX_TRAJ_VERTS);
     vz = (float *) TMP_MALLOC (2 * sizeof (float) * MAX_TRAJ_VERTS);
     nx = (float *) TMP_MALLOC (2 * sizeof (float) * MAX_TRAJ_VERTS);
     ny = (float *) TMP_MALLOC (2 * sizeof (float) * MAX_TRAJ_VERTS);
     nz = (float *) TMP_MALLOC (2 * sizeof (float) * MAX_TRAJ_VERTS);
     tt = (int *) TMP_MALLOC (2 * sizeof (int) * MAX_TRAJ_VERTS);
   }
   else {
     vx = (float *) TMP_MALLOC (sizeof (float) * MAX_TRAJ_VERTS);
     vy = (float *) TMP_MALLOC (sizeof (float) * MAX_TRAJ_VERTS);
...
/* 30Jan06  Phil McDonald */
         mem_free (ctx->dpy_ctx->topo->TopoIndexes[time]);
/* end PM */
...
/* 30Jan06  Phil McDonald */
      ...
      indexes = mem_malloc (trows * tcols * sizeof(uint_1));
/* end PM */
      if (!indexes){
...
/* 05Jun07  Phil McDonald	be consistent with extract_sfc_slice */
            xyzPRIME_to_geo (dtx, -1, -1, x, y, z, &lat, &lon, &hgt);
            value = interp_sfc_val (ctx, ctxtime, dtx->topo->TopoColorVar,
                                    lat, lon, rcl_max);
/* end PM */
...
/* 05Apr06  Phil McDonald */
    if ((type != TASK_HWIND) && (type != TASK_HSTREAM) &&\
        (type != TASK_VWIND) && (type != TASK_VSTREAM))
    {
/* end PM */
