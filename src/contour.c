...
/* 08Mar06  Phil McDonald	if we're going to bail out, do it now */
...
/* 31Jan06  Phil McDonald */
   vx   = (float*) TMP_MALLOC (sizeof (float) * maxtemp);
   vy   = (float*) TMP_MALLOC (sizeof (float) * maxtemp);
   ipnt = (int*) TMP_MALLOC (sizeof (int) * ((nr-1) * (nc-1) + 1));
   /* see below loop */
   if (!vx || !vy || !ipnt)
   {
...
/* 31Jan06  Phil McDonald	eliminate unnecessary trailing zeroes */
   if ((float)(int) interval == interval) strcpy (lbl_fmt, "%.f");
/* end PM */
...
/* end PM */
   ...
   ip = 0;
/* 08Mar06  Phil McDonald	corrected limit on ip */
   for (ir = 0; (ir < nrm) && (ip < nump); ir++) {
      for (ic = 0; (ic < ncm) && (ip < nump); ic++) {
         int start, len;
         start = ipnt[ip];
         len = ipnt[ip+1] - start;
         if (len>0) {
