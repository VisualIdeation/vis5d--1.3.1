...
/* 01Feb06  Phil McDonald */
      r->SampLev = (float *) TMP_MALLOC
                   (r->inR * r->inC * r->outL * sizeof (float));
		
      if(topo==NULL){
		  topo = (struct Topo *) TMP_CALLOC (1, sizeof (struct Topo));
/* end PM */
		}
   ...
   if (r->inproj != r->outproj) {
      ...
      r->DoHorizontal = 1;
/* 01Feb06  Phil McDonald */
      r->SampRow = (float *) TMP_MALLOC (r->outR * r->outC * sizeof (float));
      r->SampCol = (float *) TMP_MALLOC (r->outR * r->outC * sizeof (float));
/* end PM */
                                 ...
                                 int outnl )
{
   ...
   if (NumResamplers<MAX_RESAMPLERS) {
      struct resampler *r;
/* 01Feb06  Phil McDonald */
      r = (struct resampler *) TMP_MALLOC (sizeof (struct resampler));
/* end PM */
      r->inproj = inproj;
      r->invcs = invcs;
      r->outproj = outproj;
...
/* 01Feb06  Phil McDonald */
      if (ResamplerList[i]->DoVertical) {
         TMP_FREE (ResamplerList[i]->SampLev);
      }
      if (ResamplerList[i]->DoHorizontal) {
         TMP_FREE (ResamplerList[i]->SampRow);
         TMP_FREE (ResamplerList[i]->SampCol);
      }
      TMP_FREE (ResamplerList[i]);
/* end PM */
   }
