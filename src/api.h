...
extern int vis5d_set_cursor_type (Display *dpy, Window win, int shape);
/* end PM */
...
extern int	vis5d_mem_set_funcs (void *(*calloc_func) (size_t, size_t),
		                     void *(*malloc_func) (size_t),
		                     void (*free_func) (void *));

extern void	*vis5d_mem_calloc (size_t nmemb, size_t size);
extern void	*vis5d_mem_malloc (size_t size);
extern void	vis5d_mem_free (void *ptr);

/* 01Feb06  Phil McDonald */
extern char*	vis5d_strdup (char *str);

/* 14Feb06  Phil McDonald */
extern int	vis5d_debug_msg (char* msg);

/* 22May06  Phil McDonald */
extern int	vis5d_make_volume (int disp_index);

/* 13Sep06  Phil McDonald */
extern int	vis5d_debug_msg2 (char *str1, char *str2);

/* 11Sep06  Phil McDonald */
extern double	vis5d_time_usec (double usec_beg);

/* 13Sep06  Phil McDonald */
extern int	vis5d_time_msg (double usec_beg, char *str1, char *str2);

/* 26Feb07  Phil McDonald */
extern int	vis5d_set_volume_dims (int disp_index, int nx, int ny, int nz);

/* 23Apr07  Phil McDonald */
#ifdef HAVE_PTHREADS
#  include <pthread.h>

extern int	vis5d_sync_set_funcs (
...
extern int	vis5d_set_topo_hires_maxverts (int disp_index, int maxverts);
/* end PM */
