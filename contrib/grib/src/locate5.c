...
int bword (void *, int loc, int n);	/* return n bytes at location loc */
void gsdate (int  day, int  mon, int  year, int *serial); /* serial date */
...
int locate5 (int ixf)
{
 ...
 int time_range;
 int serial_date;	/* serial date sence 1 Jan 1990 from gsdate */

 if (bug > 3) fprintf (stderr, "in locate5 file number: %2d\n",ixf);
 if (bug > 3) fprintf (stderr, "process file %s\n",want.file[ixf]);
 if (bug > 3) fprintf (stderr, "grid id %d\n", want.grid_id[ixf]);  /* DEBUG */
 ...
 if (bug > 3) {fprintf (stderr, "in locate5\n");}
 while (eof_flag)
 {
  ...
  for (j=0; j<n_read; j++)
   {if (strncmp((char *)buf+j, "GRIB", 4) == 0)
	...
	fprintf (stderr, "lenpds = %d %8.8x\n", lenpds, lenpds);
     flag_gds_bms = bword ( (char *)buf, locpds+7, 1);
	fprintf (stderr, "flag_gds_bms = %d %8.8x   %d %8.8x\n", flag_gds_bms, flag_gds_bms, 0x00000080, 0x00000080);
	...
	fprintf (stderr, "year = %d %8.8x\n", year, year);
     ...
     if (bug > 3)
      {fprintf (stderr, "%3d %6d %8d %6d %8d ",ix_grid,j,loc,len,loc_next); /* DEBUG */
       fprintf (stderr, "%.4s", (char *)buf+j); /* DEBUG */
       fprintf (stderr, "%4d %4d %4d %5d ",grid_id,i_param,i_level,level);
       fprintf (stderr, "%2d %2d %2d %3d %3d %3d\n",
            year,month,day,hour,fcst_un,per1); }	/* DEBUG */	
	fprintf (stderr, "want.grid_id %d %d %d\n", ixf, want.grid_id[ixf], grid_id);
	...
	fprintf (stderr, "want.fcst_hr %d %d %d\n", ixf, want.fcst_hr[ixf], per1);
     if (want.fcst_hr[ixf] != per1) continue;
     for (i=0; i<NVAR; i++)
	fprintf (stderr, "var %d %d   %d %d   %d %d\n", i, NVAR, i_param, want.vars[i], i_level, want.lev_ind[i]);
      {if (i_param == want.vars[i]  &&  i_level == want.lev_ind[i])
        {if (bug>3) fprintf (stderr, "want this grid %2d %3d %4d %8ld %8d %6d\n",
         ...
         if (igd > NGRID-2)
          {fprintf (stderr, "NGRID exceeded in locate5\n"); close(fdi); return -2;}
         if (len > BUFF*4)
          {fprintf (stderr, "locate: grid record size %d > %d read buffer\n",
            len, BUFF*4);
           continue; }
         igd++;
 ...
 close (fdi);
 if (bug > 3) fprintf (stderr, "exit locate5\n"); if (bug > 3) fflush(NULL); /* DEBUG */
 return 0;
}
